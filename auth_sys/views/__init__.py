...
from auth_sys.forms import RegistrationForm
from auth_sys.validators import get_default_scopes
import logging
...
class CustomAuthorizationView(AuthorizationView):
    ...
    def __init__(self, **kwds):
        super(CustomAuthorizationView, self).__init__(**kwds)
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            try:
                ...
                # Removing oauthlib.Request from credentials. This is not
                # required in future
            except KeyError:  # pylint: disable=pointless-except
                pass

            kwargs['scopes_descriptions'] = [oauth2_settings.SCOPES[scope]
                                             for scope in scopes]
            kwargs['scopes'] = scopes
            # at this point we know an Application instance with such client_id
            ...
            # following two loc are here only because of
            # https://code.djangoproject.com/ticket/17795
            form = self.get_form(self.get_form_class())
            kwargs['form'] = form

            # Check to see if the user has already granted access and return
            # a successful response depending on 'approval_prompt' url
            # parameter
            ...
            if application.skip_authorization:
                ...
                tokens = request.user.oauth2_provider_accesstoken.filter(
                    application=kwargs['application']).all().order_by('-id')
                if len(tokens) > 0:
                    token = tokens[0]
                    if len(tokens) > 1:
                        # Enforce one token pair per user policy. Remove all
                        # older tokens
                        request.user.oauth2_provider_accesstoken.exclude(
                            pk=token.id).all().delete()

                    # check past authorizations regarded the same scopes as the
                    # current one
            ...
            self.logger.exception("fail to authorize")
