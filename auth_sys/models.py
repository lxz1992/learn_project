...
class Application(AbstractApplication):
    ...
    private_scopes = models.CharField(
        max_length=256, help_text='Private API scopes', null=True, blank=True)
    website = models.URLField(null=False, blank=False)
    privacy_policy = models.URLField(
        null=True, blank=True, help_text='Link of privacy policy of application')
    ...
    def clean(self):
        ...
        if self.authorization_grant_type == AbstractApplication.GRANT_PASSWORD:
            error = _(
                "Resource owner password based grants are not allowed for now")
