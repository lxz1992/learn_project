...
class CustomOAuth2Validator(OAuth2Validator):
    ...
    def get_default_scopes(self, client_id, request, *args, **kwargs):
        application = get_object_or_404(
            get_oauth2_application_model(), client_id=client_id)
        return get_default_scopes(application)

    def validate_scopes(self, client_id, scopes, client, request, *args, **kwargs):
        request.scopes = list(set(request.scopes).union(
            set(get_default_scopes(client))))
        return super(CustomOAuth2Validator, self).validate_scopes(client_id, scopes, client, request, *args, **kwargs)

    def save_bearer_token(self, token, request, *args, **kwargs):
        if request.refresh_token:
            # remove used refresh token
            # Copied as is from parent. I don't know why they're even caring to
            # delete this! - Dheerendra
            try:
